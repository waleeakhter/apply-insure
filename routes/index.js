...
// SendGrid settings with nodemailer
var options = {
  auth: {
...
router.post("/get_ethoslife", async function (req, res, next) {
...
router.post("/get_zillow", async (req, response, next) => {
  ...
  if (address) {
    ...
    await req.end(async function (res) {
      ...
      const { mpr_id } =
        (res.body.autocomplete && res.body.autocomplete[0]) || null;
...
router.post("/get_statistics", async function (req, res, next) {
  ...
  async function runQueries(link, is_admin) {
    ...
    async function getResult(links) {
      ...
      const applicantsQuery = Applicant.distinct("link", {
        link: { $in: applicantsCondition },
      });
      ...
      if (applicants.length > 0) {
        await applicants.forEach(async function (applicant, index) {
          ...
          let countCYTDQuery = Applicant.count({
            link: applicant,
            register_date: {
              $gte: new Date(dateYTD.setFullYear(currentYear - 1)),
              $lt: cdate,
            },
          });
          ...
          let countC90Query = Applicant.count({
            link: applicant,
            register_date: {
              $gte: new Date(new Date(date90).setFullYear(currentYear - 1)),
              $lt: cdate,
            },
          });
          ...
          let countC30Query = Applicant.count({
            link: applicant,
            register_date: {
              $gte: new Date(new Date(date30).setFullYear(currentYear - 1)),
              $lt: cdate,
            },
          });
          ...
          let countC7Query = Applicant.count({
            link: applicant,
            register_date: {
              $gte: new Date(new Date(date7).setFullYear(currentYear - 1)),
              $lt: cdate,
            },
          });
          ...
          let countC1Query = Applicant.count({
            ...
            register_date: {
              $gte: new Date(new Date(date1).setFullYear(currentYear - 1)),
              $lt: cdate,
            },
          });
...
router.post("/send_more_email", function (req, res, next) {
  let { address, persons, type, agent, basicPrice, goodPrice, enhancedPrice } =
    req.body;
  persons = JSON.parse(persons);
  let subject1 =
    config.agentsInfo[agent["email"]]["name"] +
    " **NQR* - " +
    address.split(",")[0] +
    " - " +
    (persons[0]["first_name"] + " " + persons[0]["last_name"]);
...
router.post("/send_life_email", function (req, res, next) {
  ...
  if (total_data.manualAddress == "") {
    if (total_data.address) {
      ...
      address =
        ...
        ", " +
        total_data.address.locality +
        ", " +
        total_data.address.administrative_area_level_1;
  ...
  for (var i = 0; i < total_data.personData.length; i++) {
    ...
    total_data.personData[i].birthday =
  ...
  if (total_data.insurence_want != "ASAP") {
    ...
    total_data.insurence_want =
      ("0" + (insurence_want.getMonth() + 1)).slice(-2) +
      "-" +
      ("0" + insurence_want.getDate()).slice(-2) +
      "-" +
  ...
  if (total_data.personData.length) {
    subject1 =
      total_data.agent["name"] +
      " **NQR* - " +
      address +
      " - " +
      (total_data.personData[0]["first_name"] +
        " " +
        total_data.personData[0]["last_name"]);
  } else {
  ...
  total_data.agent["logo"] =
    "https://apply.insure/assets/images/agents/" + total_data.agent["logo"];
  Object.assign(total_data, { agentEmail });
  Object.assign(total_data, { agentPhone });
  Object.assign(total_data, { agentSubject });

  total_data.applicant_email_content = total_data.applicant_email_content
    .split("<br>")
    .join("\n");
  ...
  let additional_email =
    total_data.agent["additional_email"].trim() != ""
      ? total_data.agent["additional_email"].replace("|", ",")
      : "";
  ...
  for (let i = 0; i < total_data.carData.length; i++) {
    car_data +=
      ...
      (i + 1) +
      ")  " +
      total_data.carData[i]["year"] +
      "  " +
      total_data.carData[i]["type"] +
      "  " +
      total_data.carData[i]["model"] +
      "\n";
  }
  ...
  if (spreadsheet_id != "" && short_app_sheet_id != "") {
    ...
    const qs = {
      lead: {
        ...
        basement_finished:
          total_data.discountsData.claim_free == true ? "Yes" : "No",
        security_system:
          total_data.discountsData.security_system == true ? "Yes" : "No",
...
router.post("/send_advance_life_email", function (req, res, next) {
  ...
  // let car_data_divs = "";
  // for(let i = 0;i < total_data.carData.length;i++){
  //   car_data_divs += '<tr style="height:20px;background-color:'+ ((i % 2)==0 ? 'lightgrey':'white') +'"><td style="text-align: center;width: 33%;vertical-align: middle;"><div style="font-size: 14px;">'+total_data.carData[i]['year']+'</div></td><td style="text-align: center;width: 33%;vertical-align: middle;"><div style="font-size: 14px;">'+total_data.carData[i]['type']+'</div></td><td style="text-align: center;width: 33%;vertical-align: middle;"><div style="font-size: 14px;">'+total_data.carData[i]['model']+'</div></td></tr>';
  // }
  // let tempHomeType = total_data.hometype;
  // total_data.hometype = car_data_divs;
  //  total_data.personData[0].birthday = total_data.personData[0].birthday;
  ...
  if (total_data.pageFiveCustomQuestion.length > 0) {
    for (let i = 0; i < total_data.pageFiveCustomQuestion.length; i++) {
      fifthPageCustomQuestion +=
        " (" +
        (i + 1) +
        ")  " +
        total_data.pageFiveCustomQuestion[i]["question"] +
        ":  " +
        total_data.pageFiveCustomQuestion[i]["answer"] +
        "\n";
    }
  }
  ...
  if (total_data.pageSixCustomQuestion.length > 0) {
    for (let i = 0; i < total_data.pageSixCustomQuestion.length; i++) {
      sixPageCustomQuestion +=
        " (" +
        (i + 1) +
        ")  " +
        total_data.pageSixCustomQuestion[i]["question"] +
        ":  " +
        total_data.pageSixCustomQuestion[i]["answer"] +
        "\n";
    }
  }
  ...
  let birthday =
    ...
    "-" +
    dateofbirth.getFullYear();
  //total_data.hometype = tempHomeType;
  // check spreadsheet & short app sheet id availabel the zapier code execute.
  if (spreadsheet_id != "" && full_app_sheet_id != "") {
    ...
    const qs = {
      lead: {
        ...
        short_term_rental:
          total_data.propertyData.term_rental == true ? "Yes" : "No",
        property_own:
          total_data.propertyData.property_own == true ? "Yes" : "No",
        multiple_unit:
          total_data.propertyData.multiple_unit == true ? "Yes" : "No",
        insurance_claims:
          total_data.propertyData.insurance_claims == true ? "Yes" : "No",
        policy_renewed:
          total_data.propertyData.policy_renewed == true ? "Yes" : "No",
        swimming_pool:
          total_data.propertyData.swimming_pool == true ? "Yes" : "No",
        car_data: car_data,
        carrier_type: total_data.carrier_type,
        insurence_want: total_data.insurence_want,
        requestorcomments: total_data.requestorcomments,
  ...
  function (req, res, next) {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      for (let i = 0; i < total_data.file_length; i++) {
        attachament_arr.push({
          filename: files["uploadedDocument_" + i]["name"],
          path: files["uploadedDocument_" + i]["path"],
        });
      }
...
router.post(
  "/send_adavace_life_upload_doc_email",
  cors({
    origin: true,
    "Access-Control-Allow-Credentials": true,
    "Access-Control-Allow-Origin": true,
    ...
    extIP.get().then(
      (ip) => {
        ...
        form.parse(req, async function (err, fields, files) {
          let total_data = JSON.parse(fields.data);
          total_data.ipAddress = ipAddress
          const persons = total_data.personData;
          ...
          if (total_data.quote_id == "" || total_data.quote_id == undefined) {
            total_data.quote_id =
              "NQR000" +
              Date.now().toString().substr(6, Date.now().toString().length);
          }
          if (
            total_data.manualAddress == "" ||
            total_data.manualAddress == undefined
          ) {
            console.log(total_data.address)
            if (total_data.address) {
              address =
                total_data.address.address +
                ", " +
          ...
          await Applicant.find(
            ...
            async function (err, applicant) {
              ...
              if (total_data.personData.length) {
                ...
                applicant.agentemail = total_data.agentemail
                applicant.agent.push({
                  email: total_data.agent.email,
                  name: total_data.agent.name,
                  phone: total_data.agent.phone,
                });
                applicant.insuranceType = total_data.insuranceType
                  ? total_data.insuranceType
                  : "";
                applicant.applicantIP = ipAddress;
                applicant.riskManagement = total_data.riskManagement;
                applicant.policyStartDate = total_data.policyStartDate;
                applicant.riskManagementQuestions = total_data.riskManagementQuestions,
                  applicant.link = total_data.agent.link;
                applicant.status = "0";
                if (total_data.pageFiveCustomQuestion.length > 0) {
                  applicant.question5 =
                    total_data.pageFiveCustomQuestion[0]["question"];
                  applicant.answer5 =
                    total_data.pageFiveCustomQuestion[0]["answer"];
                } else {
                  applicant.question5 = "";
                  applicant.answer5 = "";
                }
                if (total_data.pageSixCustomQuestion.length > 0) {
                  applicant.question6 =
                    total_data.pageSixCustomQuestion[0]["question"];
                  applicant.answer6 =
                    total_data.pageSixCustomQuestion[0]["answer"];
                } else {
                  applicant.question6 = "";
                  applicant.answer6 = "";
                }
                applicant.carrierType = total_data.carrier_type
                  ? total_data.carrier_type
                  : "";
                applicant.requestorcomments = total_data.requestorcomments
                  ? total_data.requestorcomments
                  : "";
                applicant.mailingadress = total_data.mailing_address.address
                  ? total_data.mailing_address.address
                  : total_data.mailing_address;
                ...
                applicant.current_auto_premiun = total_data.current_auto_premium
                  ? total_data.current_auto_premium
                  : "";
                ...
                applicant.cardatayear = total_data.carData[0]["year"]
                  ? total_data.carData[0]["year"]
                  : "";
                applicant.cartype = total_data.carData[0]["type"]
                  ? total_data.carData[0]["type"]
                  : "";
                applicant.carmodel = total_data.carData[0]["model"]
                  ? total_data.carData[0]["model"]
                  : "";
                applicant.shortrentals = total_data.propertyData["term_rental"];
                applicant.propertyown = total_data.propertyData["property_own"];
                applicant.multipleunit =
                  total_data.propertyData["multiple_unit"];
                applicant.insuranceclaims =
                  total_data.propertyData["insurance_claims"];
                applicant.policycancell =
                  total_data.propertyData["policy_renewed"];
                applicant.swimmingpool =
                  total_data.propertyData["swimming_pool"];
                applicant.security_system =
                  total_data.discountsData["security_system"];
          ...
          //  total_data.personData[0].birthday = total_data.personData[0].birthday;
          const detailsEmailTemplate = swig.compileFile(
            "advance_details_email.twig"
          );
          console.log(total_data)
          total_data.policyStartDate = moment(total_data.policyStartDate).format('MMMM Do YYYY');
          const html1 = detailsEmailTemplate(total_data);
          fs.writeFileSync("./detailsEmailTemplate123.html", html1);
          ...
          total_data.agent["logo"] =
            "https://apply.insure/assets/images/agents/" +
            total_data.agent["logo"];
          ...
          total_data.applicant_email_content =
            total_data.applicant_email_content.split("<br>").join("\n");
          const thankyouTemplate = swig.compileFile("thankyou.twig");
          const html2 = thankyouTemplate(total_data);
...
router.post("/send_signup_request", async function (req, res, next) {
  ...
  await bcrypt.genSalt(10, async function (err, salt) {
    bcrypt.hash(newUserModel.password, salt, async function (err, hash) {
  ...
  const htmlPassword = "<div style='align-center:center'><h1>Login Password</h1><h2>" + defaultpassword + "</h2></div>"
  const msg1 = {
    from: config.from,
    to: [to],
  ...
  const passowrdMsg = {
    ...
    subject: "Apply Insure: Login Password",
    html: htmlPassword,
  };
  let mail2 = transporter.sendMail(msg1, function (err, info) {
    if (err) {
  ...
  let passowrdMail = transporter.sendMail(passowrdMsg, function (err, info) {
    if (err) {
      console.log(err);
    } else {
      console.log(info);
  ...
  try {
    ...
    if (params.subject.manualAddress == "") {
      if (params.subject.address) {
        address =
          params.subject.address.address +
          ", " +
          params.subject.address.locality +
          ", " +
          params.subject.address.administrative_area_level_1;
      } else {
    ...
    if (params.subject.personData.length) {
      subject =
        params.subject.agentName +
        " **NQR* - " +
        address +
        " - " +
        (params.subject.personData[0]["first_name"] +
          " " +
          params.subject.personData[0]["last_name"]);
    } else {
    ...
    for (var i = 0; i < params.eightPlusPerson; i++) {
      html +=
        `<tr>
                      <th style="text-align:left;">DL Name  ` +
        (i + 1) +
        `: </th>
                      <td>` +
        params.AccurateQuoteFormData["DLName" + i] +
        `</td>
                    </tr>`;
    }
    ...
    for (var i = 0; i < params.carDataLength; i++) {
      html +=
        `<tr>
                      <th style="text-align:left;">CAR VIN ` +
        (i + 1) +
        `: </th>
                      <td>` +
        params.AccurateQuoteFormData["CARVIN" + i] +
        `</td>
                    </tr>`;
    }
    html += `</tbody>
...
router.get("/zapier/agent-login", function (req, res) {
  try {
    con.connect(function (err) {
      ...
      str = str.replace(")", "");
      con.query(
        "SELECT * FROM agents WHERE username = '" + str + "' ",
      ...
      mv(oldpath, newpath, { mkdirp: true }, function (err) {
        if (err) {
          ...
          let htmlbody =
            ...
            "                  </table>\n";
          let htmlContent =
            commonHelper.getEmailCommonPart()["header"] +
            htmlbody +
            commonHelper.getEmailCommonPart()["footer"];
    ...
    if (propertyId) {
      ...
      req.end(async function (res) {
        ...
        if (res.body && res.body.properties && res.body.properties.length > 0) {
          const resp = {
            year_built:
              res.body.properties[0].year_built ||
              res.body.properties[0].public_records[0].year_built,
            price: res.body.properties[0].price,
            building_size:
              res.body.properties[0].building_size.size ||
              res.body.properties[0].public_records[0].building_sqft,
          };
          resolve(resp);
        }
      });
